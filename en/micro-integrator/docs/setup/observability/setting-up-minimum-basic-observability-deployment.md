...
### Setting up the cloud native observability solution logging and tracing capabilities
...
3. Open the `<PATH>/values.yaml` file and set the `enabled` parameter to `true` for both Loki-stack and Jaeger as shown in the extract below.
